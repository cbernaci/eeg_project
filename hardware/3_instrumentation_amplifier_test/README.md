...
Here I document the circuits tested at each step while building the EEG. I wanted to perform tests
at different stages, starting with testing the SW data capture, then a basic instrumentation amplifier 
circuit built from scratch, then using either a ready-made amplification board (e.g. openBCI Cyton, TI ADS1299 
Evaluation Board) for collecting EEG data. 

microp before going further so I used the Arduino Uno to generate a
...
1. square wave only circuit for SW data capture
